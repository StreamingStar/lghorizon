...
class LGHorizonMediaPlayer(MediaPlayerEntity):
    ...
    def __init__(
        ...
        self.entry = entry
        self._channels = {}
        self._last_good_linear_metadata: dict[str, object] = {}

     def _refresh_linear_metadata_cache(self) -> None:
        """Cache complete linear metadata to smooth provider fallback updates."""
        state = self._device.device_state

        channel = (state.channel_name or "").strip()
        title = (state.show_title or "").strip()
        image = (state.image or "").lower()

        # Check for invalid states or launcher before updating the cache
        if channel in {"", "No Channel"}:
            return
        if title in {"", "No Program"}:
            return
        if "launcher" in title.lower():  # Keep previous cache if on launcher
            return
    ...
    def _should_use_cached_linear_metadata(self) -> bool:
        """Use cached metadata when linear updates degrade or launcher is active."""
        state = self._device.device_state
        ...
        degraded_channel = channel in {"", "No Channel"}
        # "launcher" check here ensures we stick to the cache when on the home screen
        degraded_title = title in {"", "No Program"} or "launcher" in title.lower()
        degraded_timing = not state.duration and not state.position
        degraded_image = "appstore" in image

        return degraded_channel or degraded_title or degraded_timing or degraded_image

    def _get_metadata_value(self, key: str):
        """Return metadata value with linear fallback smoothing."""
        self._refresh_linear_metadata_cache()
        if self._should_use_cached_linear_metadata():
            return self._last_good_linear_metadata.get(key)
        return getattr(self._device.device_state, key)
    ...
    def media_duration(self) -> int | None:
        ...
        duration = self._get_metadata_value("duration")
        if not duration:
            return 0
    ...
    def media_episode(self) -> str | None:
        """Return the media type."""
        episode_number = self._get_metadata_value("episode_number")
        if episode_number:
    ...
    def media_image_url(self):
        ...
        image_url = self._get_metadata_value("image")
    ...
    def media_position(self) -> int | None:
        ...
        position = self._get_metadata_value("position")
        if not position:
            return None
        return position
    ...
    def media_position_updated_at(self) -> dt.datetime | None:
        ...
        last_position_update = self._get_metadata_value("last_position_update")
        return dt_util.utc_from_timestamp(
            int(last_position_update or 0)
        )

    @property
    def media_season(self) -> str | None:
        """Return the media title."""
        season_number = self._get_metadata_value("season_number")
        if season_number:
    ...
    def media_series_title(self):
        """Return the media title."""
        return self._get_metadata_value("episode_title") or ""

    @property
    def media_title(self):
        """Return the media title."""
        return self._get_metadata_value("show_title")

    @property
    def source(self):
        """Name of the current channel."""
        return self._get_metadata_value("channel_name")
